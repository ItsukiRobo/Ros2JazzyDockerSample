...
その後、action `/cylinder_controller/track_sine_force` に goal を送ると `SINE_TRACKING` モードへ切り替わり、loadcell の force が目標 sin 波に追従するように制御します。
...
```bash
ros2 action send_goal /cylinder_controller/track_sine_force controller/action/TrackSineForce "{amplitude_n: 50.0, offset_n: 0.0, frequency_hz: 0.5, duration_s: 10.0, phase_rad: 0.0}"
```
